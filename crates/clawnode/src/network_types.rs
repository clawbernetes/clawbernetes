//! Network mesh types.
//!
//! Provides WireGuard mesh topology, IP allocation, and node registration
//! types that were previously in the phantom `claw_network` crate.
...
/// Simple non-crypto random u64 using thread_rng-like approach.
fn rand_u64() -> u64 {
...
pub struct IpAllocator {
    /// Per-region next node IP counter.
    region_counters: Mutex<HashMap<Region, u32>>,
    /// Global workload subnet counter.
...
impl IpAllocator {
    ...
    pub fn allocate_node_ip(&self, region: Region) -> Result<IpAddr, String> {
        ...
        let counter = counters.entry(region).or_insert(1); // Start at .1
        if *counter >= 4094 {
    ...
    /// Allocate a /24 workload subnet for a node's containers.
    ///
    /// Subnets are allocated from 10.200.0.0/16.
...
///
/// Tracks all registered nodes, allocates IPs, and provides
/// the topology information needed for peer configuration.
...
/// Topology snapshot for status queries.
#[derive(Debug, Clone)]
pub struct MeshTopology {
    pub nodes: HashMap<NodeId, MeshNode>,
}

/// Allocator stats for status queries.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn test_wireguard_key_valid() {
        let key = WireGuardKey::new(&test_pubkey());
        assert!(key.is_ok());
    }

    #[test]
    fn test_wireguard_key_invalid_base64() {
        let key = WireGuardKey::new("not-base64!!!");
        assert!(key.is_err());
    }
    ...
    fn test_wireguard_key_wrong_length() {
        ...
        let key = WireGuardKey::new(&short);
        assert!(key.is_err());
    }
    ...
    #[test]
    fn test_wireguard_mesh_add_remove() {
    ...
    fn test_wireguard_mesh_duplicate_node() {
        ...
        let result = mesh.add_node(make_node());
        assert!(result.is_err());
    }

    #[test]
    fn test_wireguard_mesh_list_nodes() {
        let mesh = WireGuardMesh::new(MeshConfig::default()).expect("mesh");
        let alloc = mesh.allocator();
